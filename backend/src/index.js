...
/**
 * Build manifest from KV keys by reversing the hash pattern
 * Pattern: path/file.hash.ext -> path/file.ext
 */
async function buildManifestFromKV(kv) {
  const manifest = {};
  let cursor;
  ...
  do {
    ...
    for (const key of result.keys) {
      const hashedName = key.name;
      // Extract original name by removing hash: file.HASH.ext -> file.ext
      const originalName = hashedName.replace(/\.([a-f0-9]{10,})\./g, '.');
...
// Cache the manifest so we don't rebuild on every request
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      try {
        ...
        // Build manifest from KV if not cached
        if (!cachedManifest) {
          cachedManifest = await buildManifestFromKV(env.__STATIC_CONTENT);
        }

        // Pass KV namespace and our built manifest
